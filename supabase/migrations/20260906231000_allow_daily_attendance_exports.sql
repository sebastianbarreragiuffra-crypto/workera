-- The dashboard exposes a one-day workbook, but the legacy download guard
-- only accepted weekly, fortnightly and payroll windows. Keep the same
-- authorization boundary while admitting a strictly one-day DIARIO window.
  ...
  case p_scope
    when 'attendance.export' then
      ...
      if p_resource_id is not null
         ...
         or p_period_type not in ('DIARIO', 'SEMANAL', 'QUINCENAL', 'MENSUAL', 'PAGO')
         or p_period_start is null
         or p_period_end is null
         or p_period_end < p_period_start
         or p_period_end - p_period_start > 62
         or (p_period_type = 'DIARIO' and p_period_end <> p_period_start) then
...
comment on function public.authorize_workforce_data_access(text, uuid, text, date, date) is
  'Gate transitorio ARCOTEX para asistencia diaria/semanal/quincenal/mensual, '
  'lotes de nomina y maestro de proveedores: exige sesion, membresia, rol, '
  'MFA, recurso/periodo valido, consume cuota y audita. No habilita multiempresa.';
