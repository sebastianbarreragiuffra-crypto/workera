...
export function ExpenseAccountingQueueForm({
  companySlug,
  reportId,
  enabled,
}: {
  ...
  return (
    <form action={action} className="flex flex-col items-end gap-1">
      ...
      <button type="submit" disabled={pending || !enabled} className="min-h-11 rounded-md bg-arcotex-navy px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">
        {!enabled ? "Integración pausada" : pending ? "Preparando…" : "Preparar salida"}
      </button>
