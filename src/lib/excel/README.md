...
El libro `.xlsx` se genera desde cero para que incorpore automáticamente altas y cambios de padrón. El estándar 2026 elimina los bloques verticales y usa tres hojas visibles sin celdas combinadas dentro de sus tablas de datos, más `_GESTORA_TECNICA` oculta y protegida para identidad y versión:
...
Las fórmulas auditables principales son `HH50 pagables finales = HH50 aprobadas automáticas + Ajuste HH50/1440`, `HH100 pagables finales = HH100 aprobadas automáticas + Ajuste HH100/1440` y `Bono total = Bono HE automático + Ajuste bono`. Las horas reales se conservan en columnas separadas y nunca se recortan por un tope. Los ajustes de tiempo se ingresan como minutos enteros firmados porque el sistema de fechas 1900 de Excel no representa de forma segura una duración negativa. Toda celda ajustada cambia automáticamente a amarillo; un ajuste sin motivo o un resultado final negativo bloquea el estado.
