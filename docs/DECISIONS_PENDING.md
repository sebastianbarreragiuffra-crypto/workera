...
- `[TÉCNICA]` **MFA — DECIDIDO, IMPLEMENTADO Y VERIFICADO EN HOSTED** en `master`. Para el alcance actual basta la única identidad OWNER activa, con dos TOTP verificados y enforcement activo; una segunda cuenta OWNER es resiliencia futura, no condición para cerrar el rollout. La autoridad `account_requires_mfa()` incluye OWNER/ADMIN de plataforma y roles laborales legacy `SUPER_ADMIN`/`ADMIN_RRHH`. No se amplía todavía a permisos RBAC tenant: hacerlo antes de proteger todos sus RPC mutativos produciría un gate de UI eludible con un JWT AAL1. El diseño completo es [docs/MFA_DESIGN.md](MFA_DESIGN.md), la evidencia es [docs/MFA_HOSTED_EVIDENCE.md](MFA_HOSTED_EVIDENCE.md) y la operación está en [docs/PLATFORM_OWNER_RUNBOOK.md](PLATFORM_OWNER_RUNBOOK.md). El ensayo break-glass permanece como gate separado y se hará con una cuenta de prueba, no sobre el único OWNER real. Dos decisiones no deben revertirse:
...
- `[REEMPLAZADA]` **Identidad provisional Gmail del OWNER**: la decisión antigua de usar un Gmail personal ya no describe el sistema. Esa identidad fue retirada del provisioning; el único OWNER activo actual es la cuenta corporativa que se verificó en hosted. Antes de producción con clientes externos sigue pendiente migrar el control plane a una identidad dedicada de un dominio propio de GESTORA, sin reactivar el Gmail antiguo ni agregar allowlists operacionales.
