...
- `[USUARIO/PRODUCTO]` **MFA — alcance DECIDIDO, implementación pendiente** (conversación de septiembre 2026): segundo factor **TOTP** (app de autenticación, nunca SMS) obligatorio para las cuentas con mayor alcance de daño: el gerente, las 2 cuentas `ADMIN_RRHH` que aprueban licencias, y la cuenta de administración de plataforma (OWNER). Los roles `SUPERVISOR_*` **quedan fuera por ahora**, pero el gate debe ser configurable (helper tipo `profileRequiresMfa(profile)`), no cableado a una lista fija — los supervisores sí toman decisiones de atraso/HH de su área que afectan liquidación, así que la extensión futura no debe implicar refactor. Preguntas todavía sin responder por el usuario: (a) ¿plazo de gracia para que las cuentas existentes se inscriban, o bloqueo inmediato al desplegar?; (b) ¿quién autoriza resetear el MFA de otra persona? — no puede hacerlo el propio usuario sin un segundo control, sería anular la protección. Prompt de implementación completo entregado para Codex (rama `feat/mfa-totp`, Supabase de worktree aislado).
