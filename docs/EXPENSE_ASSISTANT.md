...
La migración `20260904210000_expense_readonly_assistant.sql` crea el contrato SQL, la bitácora, las políticas RLS y la purga global. `20260905190000_expense_assistant_retention_audit.sql` agrega un evento por ejecución con solo resultado, conteo y ventana de retención; nunca conserva consultas, respuestas, actores ni empresas. El servicio `src/lib/expenses/assistant.ts` valida de forma estricta todos los resultados antes de mostrarlos. `/api/jobs/expense-assistant-retention` ejecuta la retención diariamente, exige el mismo `CRON_SECRET` fuerte de los demás jobs y devuelve un UUID de correlación sin filtrar errores internos. Un cambio de contrato exige actualizar migración, tipos, esquema Zod, pruebas pgTAP y pruebas de aplicación en el mismo bloque.
